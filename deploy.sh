...
# Create frontend .env
mkdir -p frontend
# Use domain if available, otherwise use IP
FRONTEND_API_URL="${FRONTEND_DOMAIN:-http://${DROPLET_IP}:8000}/api/v1"
# Remove double slashes if domain already has protocol
FRONTEND_API_URL=$(echo "$FRONTEND_API_URL" | sed 's|//api|/api|')
